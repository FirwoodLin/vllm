...
def get_samples(args, tokenizer) -> list[SampleRequest]:
    ...
    if getattr(args, "csv_path", None):
        dataset = CSVDataset(
            dataset_path=args.csv_path, disable_shuffle=args.disable_shuffle
        )
...
class CSVDataset(RandomDataset):
    """
    Dataset that reads prompt_len and output_len from a CSV file.
    """
    ...
    def load_data(self) -> None:
        if self.dataset_path is None:
            raise ValueError("dataset_path must be provided for CSVDataset.")

        self.data = pd.read_csv(self.dataset_path)
        if (
            "prompt_len" not in self.data.columns
            or "output_len" not in self.data.columns
        ):
            raise ValueError(
                "CSV must contain 'prompt_len' and 'output_len' columns."
            )
    ...
    def sample(
        ...
        **kwargs,
        ...
        for i in range(num_requests):
            row = self.data.iloc[i % data_len]
            prompt_len = min( int(row["prompt_len"]),2**18-1000)
            output_len = min(int(row["output_len"]), 2**18-prompt_len-10)

            # Generate prompt
            offset = self._rng.integers(0, vocab_size)
