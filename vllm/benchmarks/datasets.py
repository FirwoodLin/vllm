...
class RandomDataset(BenchmarkDataset):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        # Load CSV data if csv_path is provided
    ...
    def sample(
        ...
        **kwargs,
        ...
        # If CSV data is available, use it instead of sampling
        if self.csv_data is not None:
            # Read CSV_MAX_MODEL_LENGTH from environment variable
            csv_max_model_length = os.environ.get("CSV_MAX_MODEL_LENGTH")
            if csv_max_model_length is not None:
                try:
                    csv_max_model_length = int(csv_max_model_length)
                    logger.info(
                        "Using CSV_MAX_MODEL_LENGTH from environment: %d",
                        csv_max_model_length,
                    )
                except ValueError:
                    logger.warning(
                        "Invalid CSV_MAX_MODEL_LENGTH value: %s, ignoring",
                        csv_max_model_length,
                    )
                    csv_max_model_length = None
            
            # Sample from CSV data
            num_csv_samples = len(self.csv_data)
            if num_requests > num_csv_samples:
                # Oversample if needed - cycle through the data
                indices = np.array([i % num_csv_samples for i in range(num_requests)])
            else:
                # Use first num_requests samples sequentially
                indices = np.arange(num_requests)
            
            # Extract lengths from CSV data
            num_special = int(tokenizer.num_special_tokens_to_add())
            input_lens = np.array([
                max(1, self.csv_data[i]["prompt_len"] - num_special)
                for i in indices
            ])
            output_lens = np.array([
                max(1, self.csv_data[i]["output_len"])
                for i in indices
            ])
            
            # Apply CSV_MAX_MODEL_LENGTH limits if set
            if csv_max_model_length is not None:
                original_input_lens = input_lens.copy()
                original_output_lens = output_lens.copy()
                
                # Apply limits: input_len = min(input_len, CSV_MAX_MODEL_LENGTH - output_len - 1000)
                # and output_len = min(output_len, CSV_MAX_MODEL_LENGTH - input_len - 10)
                for i in range(len(input_lens)):
                    # First limit input_len
                    input_lens[i] = min(
                        input_lens[i],
                        csv_max_model_length - output_lens[i] - 1000
                    )
                    input_lens[i] = max(1, input_lens[i])  # Ensure at least 1
                    
                    # Then limit output_len based on potentially adjusted input_len
                    output_lens[i] = min(
                        output_lens[i],
                        csv_max_model_length - input_lens[i] - 10
                    )
                    output_lens[i] = max(1, output_lens[i])  # Ensure at least 1
                
                # Log adjustments if any were made
                num_adjusted = np.sum(
                    (input_lens != original_input_lens) | (output_lens != original_output_lens)
                )
                if num_adjusted > 0:
                    logger.info(
                        "Applied CSV_MAX_MODEL_LENGTH=%d limits: adjusted %d/%d requests",
                        csv_max_model_length,
                        num_adjusted,
                        len(input_lens),
                    )
            
            # Generate random offsets for token sequences
            offsets = self._rng.integers(0, tokenizer.vocab_size, size=num_requests)
            
            logger.info(
                "Using CSV data: sampled %d requests with input_len range [%d, %d] and output_len range [%d, %d]",
                num_requests,
                input_lens.min(),
                input_lens.max(),
                output_lens.min(),
        ...
        for i in range(num_requests):
            if self.csv_data is not None:
                ...
                inner_seq = allowed_tokens[
                    (int(offsets[i]) + i + np.arange(input_len)) % len(allowed_tokens)
                ].tolist()
                prompt_token_ids = prefix_token_ids + inner_seq
                prompt = {"prompt_token_ids": prompt_token_ids}
                total_input_len = len(prompt_token_ids)
                token_mismatch = 0
            else:
    ...
    def _load_csv_data(self) -> None:
        """
        Load prompt_len and output_len from CSV file.
        Expected CSV format:
        prompt_len,output_len,total_len,type
        408729,624,409353,long
        4314,1197,5511,short
        """
        import csv
        ...
        try:
            ...
            # Extract prompt_len and output_len columns
            self.csv_data = [
                {
            ...
            logger.info(
                "Loaded %d samples from CSV file %s",
                len(self.csv_data),
                self.csv_path,
            )
            ...
            raise ValueError(
                f"CSV file {self.csv_path} missing required column: {e}"
            )
        except ValueError as e:
            raise ValueError(
                f"Invalid data in CSV file {self.csv_path}: {e}"
            )
...
def add_dataset_parser(parser: FlexibleArgumentParser):
    ...
    random_group.add_argument(
        ...
        default=None,
        help=(
            "Path to CSV file containing prompt_len and output_len columns. "
            "When specified, overrides --random-input-len and --random-output-len. "
            "CSV format: prompt_len,output_len,total_len,type"
        ),
    )
    ...
    if args.dataset_name == "custom":
        ...
        dataset_mapping = {
            ...
            "random": lambda: RandomDataset(
                ...
                dataset_path=args.dataset_path,
                disable_shuffle=args.disable_shuffle,
                csv_path=args.random_csv_path if hasattr(args, "random_csv_path") else None,
            ).sample(
                tokenizer=tokenizer,
                num_requests=args.num_prompts,
