...
# Decode-only DP+EP launch for DeepSeek V3 1024k on 16 GPUs.
# Topology: 2 nodes x 8 GPUs, global DP=16, TP=1, EP enabled.
...
MODEL_PATH=${MODEL_PATH:-/mnt/nvme1n1/ml_research/models_4/deepseek-v3-1024k}
...
MAX_SEQS_PER_DP=${MAX_SEQS_PER_DP:-104}
MAX_NUM_BATCHED_TOKENS=${MAX_NUM_BATCHED_TOKENS:-${MAX_SEQS_PER_DP}}
GPU_MEMORY_UTILIZATION=${GPU_MEMORY_UTILIZATION:-0.9}
CP_KV_CACHE_INTERLEAVE_SIZE=${CP_KV_CACHE_INTERLEAVE_SIZE:-64}
LOG_DIR=${LOG_DIR:-.}
PROFILE_MODE=${PROFILE_MODE:-deepseek_v3_1024k_dp16ep_iqr_lex_decode}
PROFILE_BASE_DIR=${PROFILE_BASE_DIR:-/mnt/nvme1n1/ml_research/linbinbin1/vllm-dycp/dycp/profiles}

if [ -z "${VLLM_DP_DECODE_LB_POLICY:-}" ]; then
    case "${PROFILE_MODE} ${LOG_DIR}" in
        *queue*|*vllm_default*)
            export VLLM_DP_DECODE_LB_POLICY=queue
            ;;
        *)
            export VLLM_DP_DECODE_LB_POLICY=iqr_lex_decode
            ;;
    esac
fi
SCHEDULER_LABEL=${SCHEDULER_LABEL:-${VLLM_DP_DECODE_LB_POLICY}}
...
export VLLM_ATTENTION_BACKEND=${VLLM_ATTENTION_BACKEND:-FLASHMLA}
...
mkdir -p "${LOG_DIR}"
vllm serve "${MODEL_PATH}" "${args[@]}" &> "${LOG_DIR}/deepseek_v3_1024k_dp_ep_16dp_tp1_${SCHEDULER_LABEL}_node${NODE_RANK}.log" &
