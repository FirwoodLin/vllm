...
# Decode-only DP+EP launch for Qwen3-Coder-480B FP8 on 16 GPUs.
...
MAX_NUM_BATCHED_TOKENS=${MAX_NUM_BATCHED_TOKENS:-144}
MAX_SEQS_PER_DP=${MAX_SEQS_PER_DP:-${MAX_NUM_BATCHED_TOKENS}}
GPU_MEMORY_UTILIZATION=${GPU_MEMORY_UTILIZATION:-0.9}
LOG_DIR=${LOG_DIR:-.}
PROFILE_MODE=${PROFILE_MODE:-qwen_coder_480b_dp16_tp1}
...
mkdir -p "${LOG_DIR}"
vllm serve "${MODEL_PATH}" "${args[@]}" &> "${LOG_DIR}/qwen_coder_480b_dp_ep_16dp_tp1_node${NODE_RANK}.log" &
