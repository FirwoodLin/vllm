...
STRATEGY_NAME=${STRATEGY_NAME:-dycp16dp_tp1}
STRATEGY_DIR=${STRATEGY_DIR:-"${SCRIPT_DIR}/${STRATEGY_NAME}"}

# Reuse the validated Qwen3 235B DYCP topology unless a Coder-specific launch
# script is supplied through LAUNCH_DIR/LAUNCH_SCRIPT.
LAUNCH_DIR=${LAUNCH_DIR:-"${REPO_ROOT}/dycp/examples/qwen3-235b-FP8"}
LAUNCH_SCRIPT=${LAUNCH_SCRIPT:-run_qwen_235b_dycp_16dp_tp1.sh}
...
MODEL_PATH=${MODEL_PATH:-/mnt/nvme1n1/ml_research/models_cfs/Qwen3-Coder-480B-A35B-Instruct-FP8}
...
RESULT_DIR=${RESULT_DIR:-"${STRATEGY_DIR}/results"}
...
LOG_DIR=${LOG_DIR:-"${STRATEGY_DIR}/logs"}
PROFILE_MODE=${PROFILE_MODE:-qwen_coder_480b_dycp16dp_tp1}
...
Common overrides:
  ...
  DATASET_PATH=${DATASET_PATH} NUM_PROMPTS=${NUM_PROMPTS} REQUEST_RATE=${REQUEST_RATE} ${RUNNER_NAME}
  REQUEST_RATE=16 NUM_PROMPTS=4800 MAX_CONCURRENCY=0 ${RUNNER_NAME} bench
  LOAD_FORMAT=dummy ${RUNNER_NAME}
  MAX_MODEL_LEN=300000 ${RUNNER_NAME}
  GPU_MEMORY_UTILIZATION=0.56 ${RUNNER_NAME}
  MAX_NUM_BATCHED_TOKENS=144 MAX_SEQS_PER_DP=144 ${RUNNER_NAME}
  MAX_NUM_BATCHED_TOKENS=320 MAX_SEQS_PER_DP=320 CUDAGRAPH_MAX_CAPTURE_SIZE=320 ${RUNNER_NAME}
  CUDAGRAPH_CAPTURE_SIZES=2,4,8,16,32,64,128,256,320 ${RUNNER_NAME}
  RUN_GPU_CLEANUP=0 ${RUNNER_NAME} cleanup
  STRATEGY_DIR=${STRATEGY_DIR} ${RUNNER_NAME}
  LAUNCH_DIR=${LAUNCH_DIR} LAUNCH_SCRIPT=${LAUNCH_SCRIPT} ${RUNNER_NAME}
USAGE
}
...
cleanup_local() {
    ...
    pgrep -f 'VLLM::DPCoordinator' | xargs -r kill -TERM 2>/dev/null || true
    ...
    pgrep -f 'VLLM::DPCoordinator' | xargs -r kill -KILL 2>/dev/null || true
...
pgrep -f 'VLLM::DPCoordinator' | xargs -r kill -TERM 2>/dev/null
...
pgrep -f 'VLLM::DPCoordinator' | xargs -r kill -KILL 2>/dev/null
